                ...
                <div class="card-3d-wrapper">
                  <div class="card-front">
                    ...
                    <p>Por favor, informe o código de verificação e a nova senha para concluir a alteração.</p>
                      ...
                      <p class="mb-0 mt-4 text-center"><a @click="login()" class="link">Voltar para o login?</a></p>
                    </div>
                  </div>
                </div>
...
export default {
  data() {
    ...
    async submitForm() {
      ...
      try {
        const response = await axios.put('https://api-thesis-track.vercel.app/user/update-profile/', {
          email: this.loginEmail,
          codigo: this.codigo,
          novaSenha: this.novaSenha,
          confirmSenha: this.novaSenha,
          updateType: "senha", 
        });

        if (response.status === 200) {
          // Troca de senha bem-sucedida, redirecione para a página desejada
          this.$router.push('/login');
        } else {
          // Lógica para tratar outros cenários, se necessário
        }
      } catch (error) {
